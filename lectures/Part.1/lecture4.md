...
经过 MOEA 算法后，我们希望得到有限个等距排列的位于帕累托前沿上的解，进而可以根据这些解描绘出帕累托前沿的形状。
...
切比雪夫法 (Tchebycheff Method) 也是一种常用的分解方法。它通过最小化目标函数与一个理想点之间的最大距离来处理多个目标函数。

![1766735529898](lecture4.assets/1766735529898.png)

切比雪夫法的原理是：对于某一个位于帕拉托前沿上的解 $x^*$ ，应该存在一个系数 $\lambda$，使得这个解 $x^*$ 在这个子问题上是最优的。

此时这个多目标优化问题转化为的子问题是这样的：

$$
\begin{aligned}
  \min \quad & g^T (x|\lambda, z^*)\\
  \mathrm{where}\quad & g^T (x|\lambda, z^*) = \max \{
    \lambda_1 |f_1(x) - z_1^*|, \lambda_2 |f_2(x) - z_2^*|
  \} \\
  & z^* = (z_1^*, z_2^*)
\end{aligned}
$$

这里，$g^T (x|\lambda, z^*)$ 是切比雪夫函数，指的是在系数 $\lambda$ 和参照点 $z^*$ 下的目标函数。$z^*$ 是一个理想点 (Utopian Point)，它的每个分量 $z_i^*$ 是第 $i$ 个目标函数的最小值。也就是说，有
...
用更通俗的话来解释切比雪夫方法的过程，就是先用目前所有的目标函数的最小值来构造一个理想点 $z^*$，给每个目标函数赋予不同的权重大小，组成权重向量 $\lambda$。之后，优化目标就变成了 **最小化每个目标函数距离理想点的距离的最大值**，也就是 $\min(\max(\mathrm{dist}))$ 这样的东西。通过调整权重向量 $\lambda$，可以得到不同的子问题，从而覆盖整个帕累托前沿。

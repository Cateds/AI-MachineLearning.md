...
<!-- 背包问题是一个经典的组合优化问题，假设有 n 个物品和 K 个背包，每个物品有一个重量和一个价值，每个背包有一个容量限制。目标是将物品分配到背包中，使得总价值最大，同时每个背包的重量不超过其容量。 -->

<!-- 我当时要么是没睡醒，要么是大脑离线了，为什么能写出这种东西出来 -->

背包问题是一个经典的组合优化问题，假设有 n 个物品，每个物品有一个重量和一个价值，还有一个背包，背包有一个容量限制。目标是将物品放入背包中，使得总价值最大，同时背包的重量不超过其容量。

在这个情况中，解是一串二进制变量，表示每个物品是否被放入背包。目标函数是总价值，约束条件是背包的重量不能超过其容量。
...
如果一个解在它的邻域内有着最好的目标函数值，那么它就是一个局部最优解 (Local Optimal Solution)。也就是说，对于一个解 $x^*$ ，如果对于所有 $x$ 属于 $x^*$ 的邻域 $\mathcal{N}(x^*)$ ，都有 $f(x^*) \geq f(x)$ ，那么 $x^*$ 就是一个局部最优解。
...
目标函数可以分为单模态 (Unimodal) 和多模态 (Multimodal) 两种类型。单模态函数只有一个全局最优解，而多模态函数有多个局部最优解和一个全局最优解。
...
4. 确定搜索空间
   - 为每个几何变量给出上下界（由制造能力或电磁学可行性限定），例如
     $$
     l_i \in [l_\min, l_\max], \quad w_i \in [w_\min, w_\max]
     $$
